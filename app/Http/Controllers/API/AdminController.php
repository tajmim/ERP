...
=======
        $meeting_attendent = $meeting->meeting_attendent;

        $attendents = $meeting_attendent;
        $attendents = explode(",", $attendents);
    
        foreach ($attendents as $attendent) {
            $developer = developer::find($meeting_attendent);
            $developer_email = $developer->email;
            ...
            try {
                Mail::to($developer_email)->send(new SendMeetingMail($maildata));
                return response()->json(['message'=>'success']);
            } catch (Exception $e) {
